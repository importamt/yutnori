...
function optFrom(state: GameState, result: Parameters<typeof movableOptions>[1], from: string) {
  const o = movableOptions(state, result).find((x) => x.kind === 'piece' && x.from === from);
...
describe('throw input', () => {
  ...
  it('낙 after 윷 keeps the earlier result and goes to move phase', () => {
    let s = inputThrow(game(), 'yut');
    s = inputThrow(s, 'nak');
    expect(s.phase).toBe('move');
